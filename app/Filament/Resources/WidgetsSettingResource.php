    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                ->label('Merchant ID'),
                Tables\Columns\TextColumn::make('turn_on')
                ->label('Включить'),
                Tables\Columns\TextColumn::make('show_only_auth_users')
                ->label('Отображать только авторизованным пользователям'),
                Tables\Columns\TextColumn::make('how_long_display')
                ->label('Через сколько отображать виджет (секунды)'),
                Tables\Columns\TextColumn::make('yes_button_text')
                ->label('Текст кнопки «Да»'),
                Tables\Columns\TextColumn::make('cancel_button_text')
                ->label('Текст кнопки отмены'),
                Tables\Columns\TextColumn::make('link_when_click_yes')
                ->label('Ссылка при нажатии «Да»'),
                Tables\Columns\TextColumn::make('heading')
                ->label('Заголовок'),
                Tables\Columns\TextColumn::make('text_in_popup_window')
                ->label('Текст во всплывающем окне'),
            ])
            ->filters([])
            ->actions([
